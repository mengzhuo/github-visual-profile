...
<html lang="en">
    <head>
        ...
        function processData(data){
            ...
            for (var i in data){
                repo = data[i]
                newSet.push({name:repo.name, className:repo.language, size: Math.max(repo.impact, 0),
                             url:repo.html_url})
                var name = repo.language == null?"Unknown":repo.language;
                
                if (language_group[name] == undefined){
                    language_group[name] = 1;
                } else {
                    language_group[name] += 1;
                }
            }
